function openTesseract(value, max) {
    ...
    for (var i = 1; i <= toSpendModulo; i++) {
        num = 100 * Math.random();
        if (num >= 95) {
            player.tesseractBlessings.globalSpeed += 1
        } else if (num >= 90) {
            player.tesseractBlessings.talismanBonus += 1
        } else if (num >= 85) {
            player.tesseractBlessings.antELO += 1
        } else if (num >= 80) {
            player.tesseractBlessings.antSacrifice += 1
        } else if (num >= 70) {
            player.tesseractBlessings.antSpeed += 1
        } else if (num >= 60) {
            player.tesseractBlessings.obtainium += 1
        } else if (num >= 50) {
            player.tesseractBlessings.runeExp += 1
        } else if (num >= 40) {
            player.tesseractBlessings.offering += 1
        } else if (num >= 20) {
            player.tesseractBlessings.multiplier += 1
        } else {
            player.tesseractBlessings.accelerator += 1
        }
    }
