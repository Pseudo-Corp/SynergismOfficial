function openHypercube(value,max){
    ...
    //Then, the remaining hypercube will be opened, simulating the probability [RNG Element]
        for (var i = 1; i <= toSpendModulo; i++){
            num = 100 * Math.random();
            if(num >= 95){player.hypercubeBlessings.globalSpeed += 1}
            else if(num >= 90){player.hypercubeBlessings.talismanBonus += 1}
            else if(num >= 85){player.hypercubeBlessings.antELO += 1}
            else if(num >= 80){player.hypercubeBlessings.antSacrifice += 1}
            else if(num >= 70){player.hypercubeBlessings.antSpeed += 1}
            else if(num >= 60){player.hypercubeBlessings.obtainium += 1}
            else if(num >= 50){player.hypercubeBlessings.runeExp += 1}
            else if(num >= 40){player.hypercubeBlessings.offering += 1}
            else if(num >= 20){player.hypercubeBlessings.multiplier += 1}
            else{player.hypercubeBlessings.accelerator += 1}
        }
