...
function loadStatisticsCubeMultipliers() {
    ...
    const map = {
        ...
        15: {acc: 4, desc: "Bought Mortuus Est ants (R8x17):"},
        16: {acc: 4, desc: "Duplication Spirit Power:"},
        17: {acc: 5, desc: "Research 8x25:"},
        18: {acc: 4, desc: "Constant upgrade 10:"},
        19: {acc: 2, desc: "Cube upgrade 3x10:"},
        20: {acc: 2, desc: "Number Achievement 4 Bonus:"},
        21: {acc: 2, desc: "Number Achievement 6 Bonus:"}
    }
...
function loadStatisticsOfferingMultipliers() {
    ...
    const map = {
        ...
        16: {acc: 3, desc: "Cube Upgrade 5x6:"},
        17: {acc: 3, desc: "Cube Upgrade 5x10:"},
        18: {acc: 3, desc: "Platonic ALPHA:"},
        19: {acc: 3, desc: "Platonic BETA:"},
        20: {acc: 3, desc: "Platonic OMEGA:"},
    }
    for (let i = 0; i < arr.length; i++) {
...
function c15RewardUpdate(){
    let exponentRequirements = [1e3, 1.5e3, 2.5e3, 4e3, 7.5e3, 1e4, 2e4, 3e4, 5e4, 7.5e4, 1e5, 2e5, 3e5, 4e5, 5e5, 7.5e5, 1e6, 2e6, 3e6, 5e6]
    ...
    if(e >= exponentRequirements[1]){
        challenge15Rewards[keys[1]] = 1 + 1/50 * Math.pow(e/1500, 1/2)
    }
    if(e >= exponentRequirements[2]){
        challenge15Rewards[keys[2]] = 1 + 1/50 * Math.pow(e/1500,1/2)
    }
    if(e >= exponentRequirements[3]){
        challenge15Rewards[keys[3]] = 1 + 1/50 * Math.pow(e/4e3,2)
    }
    if(e >= exponentRequirements[4]){
        challenge15Rewards[keys[4]] = 1 + 1/50 * Math.pow(e/7.5e3,1/4)
    }
    if(e >= exponentRequirements[5]){
        challenge15Rewards[keys[5]] = Math.pow(1.0075, -Math.log(e)/Math.log(2))
    }
    if(e >= exponentRequirements[6]){
        challenge15Rewards[keys[6]] = Math.pow(1.006, -Math.log(e)/Math.log(2))
    }
    if(e >= exponentRequirements[7]){
        challenge15Rewards[keys[7]] = 1 + 1/5 * e/3e4
    }
    if(e >= exponentRequirements[8]){
        challenge15Rewards[keys[8]] = 1 + 1/5 * e/5e4
    }
    if(e >= exponentRequirements[9]){
        challenge15Rewards[keys[9]] = 1 + 1 * Math.pow(e/7.5e4,1.5)
    }
    if(e >= exponentRequirements[10]){
        challenge15Rewards[keys[10]] = 1 + 1/10 * Math.pow(e/1e5,1/3)
    }
    if(e >= exponentRequirements[11]){
        challenge15Rewards[keys[11]] = 1 + 1/10 * Math.pow(e/2e5,1/3)
    }
    if(e >= exponentRequirements[12]){
        challenge15Rewards[keys[12]] = 1 + 1/10 * Math.pow(e/3e5,1/2)
    }
    if(e >= exponentRequirements[13]){
        challenge15Rewards[keys[13]] = 1 + 1/20 * Math.pow(e/4e5,1/3)
    }
    if(e >= exponentRequirements[14]){
        challenge15Rewards[keys[14]] = 1 + 1/5 * Math.pow(e/5e5,1/2)
    }
    if(e >= exponentRequirements[15]){
        challenge15Rewards[keys[15]] = 1 + 1/5 * Math.pow(e/7.5e5,1/2)
    }
    if(e >= exponentRequirements[16]){
        challenge15Rewards[keys[16]] = 1 + 1/5 * Math.pow(e/1e6,1/2)
    }
    if(e >= exponentRequirements[17]){
        challenge15Rewards[keys[17]] = 1 + 1/5 * Math.pow(e/2e6,1/2)
    }
    if(e >= exponentRequirements[18]){
        challenge15Rewards[keys[18]] = 1 + 1/20 * Math.pow(e/3e6,1/3)
    }
    if(e >= exponentRequirements[19]){
        challenge15Rewards[keys[19]] = 1 + 1/10 * Math.pow(e/5e6,1/4)
    }
