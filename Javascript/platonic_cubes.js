function openPlatonic(value, max) {
    ...
    for(var i = 0; i < RNGesus.length; i++){
        let num = Math.random();
        if(toSpendModulo/40000 >= num && toSpendModulo !== 0){player.platonicBlessings[RNGesus[i]] += 1; toSpendModulo -= 1}
    }
    ...
    for(var i = 0; i < commonDrops.length; i++){
