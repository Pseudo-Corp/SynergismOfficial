    ...
    function displayRuneInformation(i,updatelevelup) {
        ...
        if (i === 3) {
            if (updatelevelup) {document.getElementById("runeshowlevelup").textContent = "~(1 + (Level/5)^2 * 2^(Level/5) / 256)x Crystal Production. +1 free level for each Crystal upgrade per 40 levels!"}
        ...
        if (i === 4) {
            if (updatelevelup) {document.getElementById("runeshowlevelup").textContent = "+0.05% building cost growth delay per level, +0.025% offering recycle chance per level [MAX: 25%], 2^((1000 - Level)/2750) Tax growth multiplier AFTER level 1000"}
        ...
        if (i === 5) {
            if (updatelevelup) {document.getElementById("runeshowlevelup").textContent = "~(1 + level/500)x Obtainium, (1 + Level^2/20000)x Ant Hatch Speed, +0.002 base offerings for each tier per level"}
...
function redeemShards(runeIndexPlusOne,auto,autoMult,cubeUpgraded) {
    ...
    if (auto){amount = Math.min(player.runeshards, 50 * Math.pow(2, player.shopUpgrades.offeringAutoLevel) * autoMult)}
