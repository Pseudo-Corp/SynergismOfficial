...
function calculateCubeMultiplier(calcMult = true) {
    let arr = [];
    arr.push(Math.pow(Math.min(1, player.ascensionCounter/10),2) * (1 + player.achievements[218] * Math.max(0, player.ascensionCounter/10 - 1)))
...
function CalcCorruptionStuff() {
    ...
    tesseractGain *= Math.pow(Math.min(1, player.ascensionCounter/10),2) * (1 + player.achievements[218] * Math.max(0, player.ascensionCounter/10 - 1))

    let hypercubeGain = (effectiveScore >= 1e9) ? 1 : 0;
    hypercubeGain *= Math.pow(1 + Math.max(0, (effectiveScore - 1e9)) / 1e8, .5);
    hypercubeGain *= Math.pow(Math.min(1, player.ascensionCounter/10),2) * (1 + player.achievements[218] * Math.max(0, player.ascensionCounter/10 - 1))
