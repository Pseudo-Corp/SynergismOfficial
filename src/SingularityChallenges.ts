...
export class SingularityChallenge {
  ...
  toString(): string {
    const color =
      this.completions === this.maxCompletions
        ? "var(--orchid-text-color)"
        : "white";
    const enabled = this.enabled
      ? `<span style="color: var(--red-text-color)">${i18next.t(
          "general.enabled"
        )}</span>`
      : "";
    return `<span style="color: gold">${this.name}</span> ${enabled}
                <span style="color: lightblue">${this.description}</span>
                <span style="color: pink">${i18next.t(
                ...
                )}: <span style="color: ${color}">${this.completions}/${
      this.maxCompletions
    }</span>
                <span style="color: gold">${i18next.t(
                  "singularityChallenge.toString.currentTierSingularity"
                )} <span style="color: var(--orchid-text-color)">${this.singularityRequirement(
      this.baseReq,
      this.completions
    )}</span></span>
  ...
  public updateChallengeHTML(): void {
    DOMCacheGetOrSet("singularityChallengesMultiline").innerHTML =
      this.toString();
  }
