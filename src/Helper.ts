...
export const addTimers = (input: TimerInput, time = 0) => {
  ...
  switch (input) {
    ...
    case "quarks": {
      ...
      player.quarkstimer =
        player.quarkstimer > maxQuarkTimer ? maxQuarkTimer : player.quarkstimer;
      break;
    }
    case "goldenQuarks": {
    ...
    case "octeracts": {
      ...
      if (player.octeractTimer >= 1) {
        const amountOfGiveaways =
          player.octeractTimer - (player.octeractTimer % 1);
        ...
        if (player.highestSingularityCount >= 160) {
          ...
          for (let i = 0; i < amountOfGiveaways; i++) {
            const quarkFraction =
              player.quarksThisSingularity * frac * actualLevel;
            player.goldenQuarks +=
              quarkFraction * calculateGoldenQuarkGain(true);
    ...
    case "ambrosia": {
      ...
      player.blueberryTime +=
        Math.floor(G.ambrosiaTimer) * player.caches.ambrosiaGeneration.totalVal;
      G.ambrosiaTimer %= 1;
      ...
      if (player.blueberryTime >= timeToAmbrosia) {
        ...
        const luckMult =
          RNG < ambrosiaLuck / 100 - Math.floor(ambrosiaLuck / 100) ? 1 : 0;
...
export const automaticTools = (input: AutoToolInput, time: number) => {
  const timeMultiplier =
    input === "runeSacrifice" || input === "addOfferings"
      ? 1
      : calculateTimeAcceleration().mult;
