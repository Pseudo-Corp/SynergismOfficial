...
const upgradetexts = [
  () => format((G.totalCoinOwned + 1) * Math.min(1e30, 1.008 ** G.totalCoinOwned), 2),
  () => format((G.totalCoinOwned + 1) * Math.min(1e30, 1.008 ** G.totalCoinOwned), 2),
  () => format((G.totalCoinOwned + 1) * Math.min(1e30, 1.008 ** G.totalCoinOwned), 2),
  () => format((G.totalCoinOwned + 1) * Math.min(1e30, 1.008 ** G.totalCoinOwned), 2),
  () => format((G.totalCoinOwned + 1) * Math.min(1e30, 1.008 ** G.totalCoinOwned), 2),
  ...
  () => format(Decimal.pow((G.totalCoinOwned + 1) * Math.min(1e30, 1.008 ** G.totalCoinOwned), 10), 2),
  () => ({
  ...
  () => null,
  () => format(1.05 ** player.achievementPoints * (player.achievementPoints + 1), 2),
  () => format((Math.min(1e25, G.totalMultiplier * G.totalAccelerator) / 1000 + 1) ** 8),
  () => format(Math.min(50, Math.floor(Decimal.log(player.transcendPoints.add(1), 1e10)))),
  () => null,
  () => format(G.totalAcceleratorBoost ** 2, 2),
  ...
  () => null,
  () => format(1 + 4 * Math.min(1, (player.maxofferings / 100000) ** 0.5), 2),
  () => format(1 + 2 * Math.min(1, (player.maxobtainium / 30000000) ** 0.5), 2),
  () => null,
  () =>
    format(
      Decimal.pow(
  ...
  () => format(1 + 0.005 * (Math.log(player.maxofferings + 1) / Math.log(10)) ** 2, 2, true),
  () => null,
  () => null,
...
export const crystalupgradedescriptions = (i: number) => {
  ...
  const q = Decimal.pow(
    ...
    G.crystalUpgradesCost[i - 1]
      + G.crystalUpgradeCostIncrement[i - 1] * Math.floor((player.crystalUpgrades[i - 1] + 0.5 - c) ** 2 / 2)
  )
