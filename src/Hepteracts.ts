...
export class HepteractCraft {
  ...
  craft = async (max = false): Promise<HepteractCraft | void> => {
    ...
    if (Number.isNaN(player.wowAbyssals) || !Number.isFinite(player.wowAbyssals) || player.wowAbyssals < 0) {
    ...
    if (Number.isNaN(amountToCraft) || !Number.isFinite(amountToCraft)) {
    ...
    // Check these lol
    if (Number.isNaN(craftAmount) || !Number.isFinite(craftAmount) || !Number.isInteger(craftAmount)) { // nan + Infinity checks
...
export const hepteractEffective = (data: hepteractTypes) => {
  ...
  if (data === 'quark') {
    ...
    if (1000 < amount && amount <= 1000 * 2 ** 10) {
      return effectiveValue * (amount / 1000) ** (1 / 2 + exponentBoost)
    } else if (1000 * 2 ** 10 < amount && amount <= 1000 * 2 ** 18) {
      return effectiveValue * (2 ** 10) ** (1 / 2 + exponentBoost)
        * (amount / (1000 * 2 ** 10)) ** (1 / 4 + exponentBoost / 2)
    } else if (1000 * 2 ** 18 < amount && amount <= 1000 * 2 ** 44) {
      return effectiveValue * (2 ** 10) ** (1 / 2 + exponentBoost)
        * (2 ** 8) ** (1 / 4 + exponentBoost / 2)
        * (amount / (1000 * 2 ** 18)) ** (1 / 6 + exponentBoost / 3)
    } else if (1000 * 2 ** 44 < amount) {
      return effectiveValue * (2 ** 10) ** (1 / 2 + exponentBoost)
        * (2 ** 8) ** (1 / 4 + exponentBoost / 2)
        * (2 ** 26) ** (1 / 6 + exponentBoost / 3)
        * (amount / (1000 * 2 ** 44)) ** (1 / 12 + exponentBoost / 6)
    }
  }
