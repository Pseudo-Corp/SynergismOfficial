...
const polyBuyToLevel = (degree: number): (budget: Decimal, baseCost: Decimal, currLevel: number, maxLevel: number) => number => {
...
const createResearchDataMap = (rangeLC: RangeLevelAndCost[], rangeU: RangeCondition[], costs: DecimalSource[], maxLevels: DecimalSource[]): Record<number, IResearchData> => {
...
export const updateResearchRoombaOLD = (index: number, auto: boolean) => {
  ...
  if (roombaResearchEnabled() && auto && player.autoResearchMode === 'cheapest') {
    player.autoResearch = researchOrderByCost[player.roombaResearchIndex]
    ...
    if (isResearchUnlocked(player.autoResearch)) {
      const idx = Math.max(researchOrderByCost[player.roombaResearchIndex], 1)
      const doc = DOMCacheGetOrSet(`res${idx}`)
      if (!isResearchMaxed(idx)) {
        doc.classList.add('researchRoomba')
      }
    }

    return
  } else if (!auto && (!roombaResearchEnabled() || player.autoResearchMode === 'manual')) {
...
export const researchDescriptions = (index: number, auto = false) => {
  ...
  let levelToBuy = getBuyableResearchLevel(index)
  levelToBuy = Math.min(researchData[index].maxLevel, levelToBuy + 1, player.researches[index] + buyAmount)

  const obtainiumCost = getCostForResearchLevels(index, levelToBuy)
