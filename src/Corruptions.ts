...
export const corruptionDisplay = (index: number) => {
  ...
  let text: Record<
    'name' | 'description' | 'current' | 'planned' | 'multiplier' | 'spiritContribution' | 'image',
    string
  > = {
  ...
  if (index < 10) {
    text = {
      ...
      multiplier: i18next.t('corruptions.scoreMultiplier', {
        curr: format(
          G.corruptionPointMultipliers[player.usedCorruptions[index] + bonusLevel] ** currentExponent,
          1
        ),
        next: format(
          G.corruptionPointMultipliers[player.prototypeCorruptions[index] + bonusLevel] ** protoExponent,
          1
        )
      }),
      spiritContribution: i18next.t('corruptions.spiritEffect', {
        curr: format(4 * (player.usedCorruptions[index] + bonusLevel) ** 2, 1),
        next: format(4 * (player.prototypeCorruptions[index] + bonusLevel) ** 2, 1)
      }),
...
async function corruptionLoadoutGetNewName (loadout = 0) {
  const maxChars = 9
  // biome-ignore lint/suspicious/noControlCharactersInRegex: I put control characters in my regex for fun!
  const regex = /^[\x00-\xFF]*$/
