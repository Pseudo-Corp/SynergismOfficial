...
export const calculateAscensionScore = () => {
    ...
    for (let i = 1; i <= 10; i++) {
        let exponent = ((i === 1 || i === 2) && player.usedCorruptions[i] >= 7 && player.platonicUpgrades[17] > 0) ? 1 + 0.15 * (player.usedCorruptions[i] - 6) + 0.01* (player.platonicUpgrades[17] - 1)*(player.usedCorruptions[i] - 7) : 1;
