...
export const calculateTotalAcceleratorBoost = () => {
  ...
  b += player.researches[93]
    * Math.floor(1 / 20 * (G.rune1level + G.rune2level + G.rune3level + G.rune4level + G.rune5level))
...
export const calculateAcceleratorMultiplier = () => {
  ...
  G.acceleratorMultiplier *= 1
    + 1 / 5 * player.researches[1] * (1 + 1 / 2 * CalcECC('ascension', player.challengecompletions[14]))
  G.acceleratorMultiplier *= 1 + 1 / 20 * player.researches[6] + 1 / 25 * player.researches[7]
    + 1 / 40 * player.researches[8] + 3 / 200 * player.researches[9] + 1 / 200 * player.researches[10]
...
export function calculateRuneExpGiven (
  ...
  const allRuneExpMultiplier = productContents([
    ...
    // Ant 8
    calculateSigmoidExponential(999, 1 / 10000 * (player.antUpgrades[8 - 1]! + G.bonusant8) ** 1.1),
    // Cube Bonus
  ...
  const droughtEffect = 1
    / G.droughtMultiplier[player.usedCorruptions[8]] ** (1 - 1 / 2 * player.platonicUpgrades[13])
...
export const lookupTableGen = (runeLevel: number) => {
  ...
  const allRuneExpRequiredMultiplier = productContents([
    ...
    Math.max(1, 1.03 ** ((runeLevel - 800) / 4))
  ])
...
export const calculateRuneExpToLevel = (runeIndex: number, runeLevel = player.runelevels[runeIndex]) => {
  ...
  if (runeIndex === 5) {
    multiplier = 100 ** runeLevel
  }
  if (runeIndex === 6) {
    multiplier = 1e25 ** runeLevel * (player.highestSingularityCount + 1)
  }
...
export function calculateOfferings (input: resetNames, calcMult = true, statistic = false) {
  ...
  if (input === 'reincarnation' || input === 'reincarnationChallenge') {
    ...
    a *= 1 + Decimal.log(player.reincarnationShards.add(1), 10) ** (2 / 3) / 4
    a *= Math.min((player.reincarnationcounter / 10 + 1) ** 2, 1)
    if (player.reincarnationcounter >= 5) {
      a *= Math.max(1, player.reincarnationcounter / 10)
    }
  }
  if (
    input === 'transcension' || input === 'transcensionChallenge' || input === 'reincarnation'
    ...
    b *= 1 + Decimal.log(player.transcendShards.add(1), 10) ** (1 / 2) / 5
    b *= 1 + CalcECC('reincarnation', player.challengecompletions[8]) / 25
    b *= Math.min((player.transcendcounter / 10) ** 2, 1)
  ...
  c *= 1 + Decimal.log(player.prestigeShards.add(1), 10) ** (1 / 2) / 5
  c *= 1 + CalcECC('reincarnation', player.challengecompletions[6]) / 50
  c *= Math.min((player.prestigecounter / 10) ** 2, 1)
  ...
  const arr = [
    ...
    1 + player.upgrades[75] * 2 * Math.min(1, (player.maxobtainium / 30000000) ** 0.5), // Particle Upgrade 3x5
    ...
    1 + (player.antUpgrades[6 - 1]! + G.bonusant6) ** .66, // Ant Upgrade:
  ...
  q *= 1.02 ** player.shopUpgrades.offeringEX3
...
export const calculateObtainium = () => {
  ...
  if (player.upgrades[69] > 0) {
    G.obtainiumGain *= Math.min(
      10,
      new Decimal(Decimal.pow(Decimal.log(G.reincarnationPointGain.add(10), 10), 0.5)).toNumber()
    )
  }
  if (player.upgrades[72] > 0) {
    G.obtainiumGain *= Math.min(
      50,
      1 + 2 * player.challengecompletions[6] + 2 * player.challengecompletions[7] + 2
          * player.challengecompletions[8]
        + 2 * player.challengecompletions[9] + 2 * player.challengecompletions[10]
    )
  }
  if (player.upgrades[74] > 0) {
    G.obtainiumGain *= 1 + 4 * Math.min(1, (player.maxofferings / 100000) ** 0.5)
  }
  ...
  G.obtainiumGain *= 1 + 0.01 * player.achievements[84] + 0.03 * player.achievements[91]
    + 0.05 * player.achievements[98] + 0.07
      * player.achievements[105]
    + 0.09 * player.achievements[112] + 0.11 * player.achievements[119] + 0.13 * player.achievements[126] + 0.15
      * player.achievements[133]
    + 0.17 * player.achievements[140] + 0.19 * player.achievements[147]
  G.obtainiumGain *= 1 + 2 * ((player.antUpgrades[10 - 1]! + G.bonusant10) / 50) ** (2 / 3)
  ...
  G.obtainiumGain *= Math.min(1, (player.reincarnationcounter / 10) ** 2)
  ...
  G.obtainiumGain *= (Decimal.log(player.transcendShards.add(1), 10) / 300) ** 2
  G.obtainiumGain = G.obtainiumGain
    ** Math.min(
      1,
      G.illiteracyPower[player.usedCorruptions[5]]
        * (1 + 9 / 100 * player.platonicUpgrades[9] * Math.min(100, Math.log10(player.researchPoints + 10)))
    )
  ...
  G.obtainiumGain *= 1.02 ** player.shopUpgrades.obtainiumEX3
  ...
  if (!Number.isFinite(G.obtainiumGain)) {
  ...
  if (player.usedCorruptions[5] >= 15) {
    G.obtainiumGain = G.obtainiumGain ** (1 / 4)
  }
  if (player.usedCorruptions[5] >= 16) {
    G.obtainiumGain = G.obtainiumGain ** (1 / 3)
  }
...
export const calculateAutomaticObtainium = () => {
  return 0.05 * (10 * player.researches[61] + 2 * player.researches[62]) * player.maxobtainiumpersecond
  ...
  if (player.highestSingularityCount < 7) {
    for (let i = 1; i <= 5; i++) {
      if (player.talismanOne[i] === (1)) {
        G.talisman1Effect[i] = (G.talismanPositiveModifier[player.talismanRarity[1 - 1]]! + positiveBonus)
          * player.talismanLevels[1 - 1] * G.challenge15Rewards.talismanBonus
      } else {
        G.talisman1Effect[i] = (G.talismanNegativeModifier[player.talismanRarity[1 - 1]]! - negativeBonus)
          * player.talismanLevels[1 - 1] * (-1) * G.challenge15Rewards.talismanBonus
      }

      if (player.talismanTwo[i] === (1)) {
        G.talisman2Effect[i] = (G.talismanPositiveModifier[player.talismanRarity[2 - 1]]! + positiveBonus)
          * player.talismanLevels[2 - 1] * G.challenge15Rewards.talismanBonus
      } else {
        G.talisman2Effect[i] = (G.talismanNegativeModifier[player.talismanRarity[2 - 1]]! - negativeBonus)
          * player.talismanLevels[2 - 1] * (-1) * G.challenge15Rewards.talismanBonus
      }

      if (player.talismanThree[i] === (1)) {
        G.talisman3Effect[i] = (G.talismanPositiveModifier[player.talismanRarity[3 - 1]]! + positiveBonus)
          * player.talismanLevels[3 - 1] * G.challenge15Rewards.talismanBonus
      } else {
        G.talisman3Effect[i] = (G.talismanNegativeModifier[player.talismanRarity[3 - 1]]! - negativeBonus)
          * player.talismanLevels[3 - 1] * (-1) * G.challenge15Rewards.talismanBonus
      }

      if (player.talismanFour[i] === (1)) {
        G.talisman4Effect[i] = (G.talismanPositiveModifier[player.talismanRarity[4 - 1]]! + positiveBonus)
          * player.talismanLevels[4 - 1] * G.challenge15Rewards.talismanBonus
      } else {
        G.talisman4Effect[i] = (G.talismanNegativeModifier[player.talismanRarity[4 - 1]]! - negativeBonus)
          * player.talismanLevels[4 - 1] * (-1) * G.challenge15Rewards.talismanBonus
      }

      if (player.talismanFive[i] === (1)) {
        G.talisman5Effect[i] = (G.talismanPositiveModifier[player.talismanRarity[5 - 1]]! + positiveBonus)
          * player.talismanLevels[5 - 1] * G.challenge15Rewards.talismanBonus
      } else {
        G.talisman5Effect[i] = (G.talismanNegativeModifier[player.talismanRarity[5 - 1]]! - negativeBonus)
          * player.talismanLevels[5 - 1] * (-1) * G.challenge15Rewards.talismanBonus
      }

      if (player.talismanSix[i] === (1)) {
        G.talisman6Effect[i] = (G.talismanPositiveModifier[player.talismanRarity[6 - 1]]! + positiveBonus)
          * player.talismanLevels[6 - 1] * G.challenge15Rewards.talismanBonus
      } else {
        G.talisman6Effect[i] = (G.talismanNegativeModifier[player.talismanRarity[6 - 1]]! - negativeBonus)
          * player.talismanLevels[6 - 1] * (-1) * G.challenge15Rewards.talismanBonus
      }

      if (player.talismanSeven[i] === (1)) {
        G.talisman7Effect[i] = (G.talismanPositiveModifier[player.talismanRarity[7 - 1]]! + positiveBonus)
          * player.talismanLevels[7 - 1] * G.challenge15Rewards.talismanBonus
      } else {
        G.talisman7Effect[i] = (G.talismanNegativeModifier[player.talismanRarity[7 - 1]]! - negativeBonus)
          * player.talismanLevels[7 - 1] * (-1) * G.challenge15Rewards.talismanBonus
      }
    }
  } else {
    for (let i = 1; i <= 5; i++) {
...
export const calculateRuneLevels = () => {
  ...
  if (player.currentChallenge.reincarnation !== 9) {
    ...
    G.rune1level = Math.max(
      1,
      player.runelevels[0] + Math.min(1e7, antUpgrade8 + G.bonusant9) * 1 + (G.rune1Talisman)
        + 7 * player.constantUpgrades[7]
    )
    G.rune2level = Math.max(
      1,
      player.runelevels[1] + Math.min(1e7, antUpgrade8 + G.bonusant9) * 1 + (G.rune2Talisman)
        + 7 * player.constantUpgrades[7]
    )
    G.rune3level = Math.max(
      1,
      player.runelevels[2] + Math.min(1e7, antUpgrade8 + G.bonusant9) * 1 + (G.rune3Talisman)
        + 7 * player.constantUpgrades[7]
    )
    G.rune4level = Math.max(
      1,
      player.runelevels[3] + Math.min(1e7, antUpgrade8 + G.bonusant9) * 1 + (G.rune4Talisman)
        + 7 * player.constantUpgrades[7]
    )
    G.rune5level = Math.max(
      1,
      player.runelevels[4] + Math.min(1e7, antUpgrade8 + G.bonusant9) * 1 + (G.rune5Talisman)
        + 7 * player.constantUpgrades[7]
    )
  }
...
export const calculateRuneBonuses = () => {
  ...
  for (let i = 1; i <= 5; i++) {
    if (G.runeBlessings[i] <= 1e30) {
      G.effectiveRuneBlessingPower[i] = (G.runeBlessings[i] ** (1 / 8)) / 75 * G.challenge15Rewards.blessingBonus
    } else if (G.runeBlessings[i] > 1e30) {
      G.effectiveRuneBlessingPower[i] = 10 ** (5 / 2) * (G.runeBlessings[i] ** (1 / 24)) / 75
        * G.challenge15Rewards.blessingBonus
    }

    if (G.runeSpirits[i] <= 1e25) {
      G.effectiveRuneSpiritPower[i] = (G.runeSpirits[i] ** (1 / 8)) / 75 * G.challenge15Rewards.spiritBonus
    } else if (G.runeSpirits[i] > 1e25) {
      G.effectiveRuneSpiritPower[i] = 10 ** (25 / 12) * (G.runeSpirits[i] ** (1 / 24)) / 75
...
export const calculateAnts = () => {
  ...
  if (player.currentChallenge.ascension === 11) {
    c11bonus = Math.floor(
      (4 * player.challengecompletions[8] + 23 * player.challengecompletions[9])
        * Math.max(0, 1 - player.challengecompletions[11] / 10)
    )
  }
  G.bonusant1 = Math.min(
    player.antUpgrades[1 - 1]! + c11,
    4 * player.researches[97] + bonusLevels + player.researches[102] + 2 * player.researches[132] + c11bonus
  )
  ...
  G.bonusant7 = Math.min(
    ...
    4 * player.researches[98] + bonusLevels + player.researches[102] + 2 * player.researches[132] + c11bonus
  )
...
const calculateAntSacrificeMultipliers = () => {
  G.timeMultiplier = Math.min(1, (player.antSacrificeTimer / 10) ** 2)
  ...
  G.upgradeMultiplier *= 1 + 2 * (1 - 2 ** (-(player.antUpgrades[11 - 1]! + G.bonusant11) / 125))
...
export const calculateAntSacrificeRewards = (): IAntSacRewards => {
  ...
  const rewards: IAntSacRewards = {
    ...
    talismanShards: (G.antELO > 500)
      ? Math.min(
        maxCap,
        Math.max(1, Math.floor(rewardsMult / 210 * (1 / 4 * (Math.max(0, G.effectiveELO - 500))) ** 2))
      )
      : 0,
    commonFragments: (G.antELO > 750)
      ? Math.min(
        maxCap,
        Math.max(1, Math.floor(rewardsMult / 110 * (1 / 9 * (Math.max(0, G.effectiveELO - 750))) ** 1.83))
      )
      : 0,
    uncommonFragments: (G.antELO > 1000)
      ? Math.min(
        maxCap,
        Math.max(1, Math.floor(rewardsMult / 170 * (1 / 16 * (Math.max(0, G.effectiveELO - 1000))) ** 1.66))
      )
      : 0,
    rareFragments: (G.antELO > 1500)
      ? Math.min(
        maxCap,
        Math.max(1, Math.floor(rewardsMult / 200 * (1 / 25 * (Math.max(0, G.effectiveELO - 1500))) ** 1.50))
      )
      : 0,
    epicFragments: (G.antELO > 2000)
      ? Math.min(
        maxCap,
        Math.max(1, Math.floor(rewardsMult / 200 * (1 / 36 * (Math.max(0, G.effectiveELO - 2000))) ** 1.33))
      )
      : 0,
    legendaryFragments: (G.antELO > 3000)
      ? Math.min(
        maxCap,
        Math.max(1, Math.floor(rewardsMult / 230 * (1 / 49 * (Math.max(0, G.effectiveELO - 3000))) ** 1.16))
      )
      : 0,
    mythicalFragments: (G.antELO > 5000)
      ? Math.min(
        maxCap,
        Math.max(1, Math.floor(rewardsMult / 220 * (1 / 64 * (Math.max(0, G.effectiveELO - 4150))) ** 1))
      )
      : 0
  }
...
export const calculateOffline = async (forceTime = 0) => {
  ...
  DOMCacheGetOrSet('offlinePrestigeTimer').innerHTML = i18next.t('offlineProgress.currentPrestigeTimer', {
    value: format(timerAdd.prestige, 2, false)
  })
  DOMCacheGetOrSet('offlineOfferingCount').innerHTML = i18next.t('offlineProgress.offeringsGenerated', {
    value: format(resetAdd.offering, 0, true)
  })
  DOMCacheGetOrSet('offlineTranscensionCount').innerHTML = i18next.t('offlineProgress.transcensionCount', {
    value: format(resetAdd.transcension, 0, true)
  })
  DOMCacheGetOrSet('offlineTranscensionTimer').innerHTML = i18next.t('offlineProgress.currentTranscensionCounter', {
    value: format(timerAdd.transcension, 2, false)
  })
  DOMCacheGetOrSet('offlineReincarnationCount').innerHTML = i18next.t('offlineProgress.reincarnationCount', {
    value: format(resetAdd.reincarnation, 0, true)
  })
  DOMCacheGetOrSet('offlineReincarnationTimer').innerHTML = i18next.t('offlineProgress.currentReincarnationTimer', {
    value: format(timerAdd.reincarnation, 2, false)
  })
  DOMCacheGetOrSet('offlineObtainiumCount').innerHTML = i18next.t('offlineProgress.obtainiumGenerated', {
    value: format(resetAdd.obtainium, 0, true)
  })
  DOMCacheGetOrSet('offlineAntTimer').innerHTML = i18next.t('offlineProgress.ingameAntSacTimer', {
    value: format(timerAdd.ants, 2, false)
  })
  DOMCacheGetOrSet('offlineRealAntTimer').innerHTML = i18next.t('offlineProgress.realAntSacTimer', {
    value: format(timerAdd.antsReal, 2, true)
  })
  DOMCacheGetOrSet('offlineAscensionTimer').innerHTML = i18next.t('offlineProgress.currentAscensionTimer', {
    value: format(timerAdd.ascension, 2, true)
  })
  DOMCacheGetOrSet('offlineQuarkCount').innerHTML = i18next.t('offlineProgress.exportQuarks', {
...
export const calculateSigmoid = (constant: number, factor: number, divisor: number) => {
  return (1 + (constant - 1) * (1 - 2 ** (-factor / divisor)))
}
...
export const calculateCubeBlessings = () => {
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    if (cubeArray[i - 1] >= 1000) {
      ...
      mult *= 1000 ** ((1 - G.blessingDRPower[i]!) * (1 + powerBonus[i - 1]))
    }
    if (i === 6) {
      power = 2.25
      mult = 1
    ...
    G.cubeBonusMultiplier[i] = Math.min(
      1e300,
      1 + mult * G.blessingbase[i]! * cubeArray[i - 1] ** (power * (1 + powerBonus[i - 1]))
...
export const calculateTotalOcteractCubeBonus = () => {
  ...
  if (player.totalWowOcteracts < 1000) {
    ...
    return 3 * (Math.log10(player.totalWowOcteracts) - 2) ** power // At 1,000 returns 3
  }
}
...
export const calculateTotalOcteractOfferingBonus = () => {
  ...
  return calculateTotalOcteractQuarkBonus() ** 1.5
}
...
export const calculateTotalOcteractObtainiumBonus = () => {
  ...
  return calculateTotalOcteractQuarkBonus() ** 1.4
}

export const calculateAllCubeMultiplier = () => {
  const arr = [
    // Ascension Time Multiplier to cubes
    Math.min(1, player.ascensionCounter / 10) ** 2 * (1 + (1 / 4 * player.achievements[204] + 1 / 4
            * player.achievements[211]
          + 1 / 2 * player.achievements[218]) * Math.max(0, player.ascensionCounter / 10 - 1)),
    // Sun and Moon achievements
    1 + 6 / 100 * player.achievements[250] + 10 / 100 * player.achievements[251],
    // Speed Achievement
    1
    + player.achievements[240]
      * Math.min(0.5, Math.max(0.1, 1 / 20 * Math.log10(calculateTimeAcceleration().mult + 0.01))),
    ...
    * G.challenge15Rewards.cube5,
    ...
    // OMEGA: C9 Cube Bonus
    1.01 ** (player.platonicUpgrades[15] * player.challengecompletions[9]),
    // Powder Bonus
    ...
    // Wow Pass INF
    1.02 ** player.shopUpgrades.seasonPassInfinity,
    // Ambrosia Mult
...
export const calculateCubeMultiplier = (score = -1) => {
  ...
  const arr = [
    ...
    * (1 + 0.4 * player.cubeUpgrades[30]), // 3x10
    ...
    1 + 4 / 100 * (player.achievements[198] + player.achievements[199] + player.achievements[200])
    ...
    1 + 1.03 ** Math.log10(Math.max(1, player.wowAbyssals)) * player.cubeUpgrades[63] - player.cubeUpgrades[63]
...
export const calculateTesseractMultiplier = (score = -1) => {
  ...
  const arr = [
    ...
    1 + 4 / 100 * (player.achievements[205] + player.achievements[206] + player.achievements[207])
...
export const calculateHypercubeMultiplier = (score = -1) => {
  ...
  const arr = [
    // Ascension Score Multiplier
    (1 + Math.max(0, score - 1e9) / 1e8) ** .5,
    // Global Multiplier
    calculateAllCubeMultiplier().mult,
    // Season Pass 2
    1 + 1.5 * player.shopUpgrades.seasonPass2 / 100,
    // Achievement 212 - 215 Bonus
    1 + 4 / 100 * (player.achievements[212] + player.achievements[213] + player.achievements[214])
...
export const calculatePlatonicMultiplier = (score = -1) => {
  ...
  const arr = [
    // Ascension Score Multiplier
    (1 + Math.max(0, score - 2.666e12) / 2.666e11) ** .75,
    // Global Multipliers
    calculateAllCubeMultiplier().mult,
    // Season Pass 2
    1 + 1.5 * player.shopUpgrades.seasonPass2 / 100,
    ...
    1 + 4 / 100 * (player.achievements[219] + player.achievements[220] + player.achievements[221])
...
export const calculateHepteractMultiplier = (score = -1) => {
  ...
  const arr = [
    // Ascension Score Multiplier
    (1 + Math.max(0, score - 1.666e16) / 3.33e16) ** 0.85,
    // Global Multiplier
    calculateAllCubeMultiplier().mult,
    // Season Pass 3
...
export const octeractGainPerSecond = () => {
  ...
  const ascensionSpeed = player.singularityUpgrades.oneMind.getEffect().bonus
    ? 10 ** (1 / 2)
    : calculateAscensionAcceleration() ** (1 / 2)
...
export const calculateOcteractMultiplier = (score = -1) => {
  ...
  const ascensionSpeedMulti = (player.singularityUpgrades.oneMind.getEffect().bonus)
    ? 10 ** (1 / 2)
      * (ascensionSpeed / 10) ** +player.octeractUpgrades.octeractOneMindImprover.getEffect().bonus
    : ascensionSpeed ** (1 / 2)
...
export const calculateTimeAcceleration = () => {
  ...
  if (corruptableTimeMult < 1) {
    const postPlat2x2 = corruptableTimeMult ** (1 - player.platonicUpgrades[7] / 30)
  ...
  const timeMult = productContents(preCorruptionArr) * productContents(corruptionArr)
...
export const calculateAscensionSpeedMultiplier = () => {
  const arr = [
    ...
    1.01 ** player.shopUpgrades.chronometerInfinity, // Chronometer INF
  ...
  const exponent = player.singularityUpgrades.singAscensionSpeed.level > 0
    ? (baseMultiplier >= 1
      ...
      : 0.97)
    : 1
  arr.push(baseMultiplier ** exponent / baseMultiplier)
...
export const calculateSingularityQuarkMilestoneMultiplier = () => {
  ...
  if (player.highestSingularityCount >= 200) {
    multiplier *= ((player.highestSingularityCount - 179) / 20) ** 2
  }
...
export const calculateCorruptionPoints = () => {
  ...
  for (let i = 1; i <= 9; i++) {
    basePoints += 16 * (player.usedCorruptions[i] + bonusLevel) ** 2
  }
...
export const calculateSummationLinear = (
  ...
  const buyToLevel = Math.min(
    ...
    Math.floor(-1 / 2 + Math.sqrt(1 / 4 + 2 * (resourceAvailable + subtractCost) / baseCost))
  )
...
export const calculateSummationNonLinear = (
  ...
  const alreadySpent = baseCost * (c * baseLevel ** 2 + baseLevel * (1 - c))
  resourceAvailable += alreadySpent
  const v = resourceAvailable / baseCost
  let buyToLevel = c > 0
    ? Math.max(0, Math.floor((c - 1) / (2 * c) + ((1 - c) ** 2 + 4 * c * v) ** (1 / 2) / (2 * c)))
    : Math.floor(v)

  buyToLevel = Math.min(buyToLevel, buyAmount + baseLevel)
  buyToLevel = Math.max(buyToLevel, baseLevel)
  let totalCost = baseCost * (c * buyToLevel ** 2 + buyToLevel * (1 - c)) - alreadySpent
...
export const solveQuadratic = (a: number, b: number, c: number, positive: boolean) => {
  ...
  const numeratorPos = -b + Math.sqrt(b ** 2 - 4 * a * c)
  const numeratorNeg = -b - Math.sqrt(b ** 2 - 4 * a * c)
...
export const calculateCubicSumData = (
  ...
  const determinantRoot = (totalToSpend / baseCost) ** 0.5 // Assume nonnegative!
...
export const computeAscensionScoreBonusMultiplier = () => {
  ...
  if (player.achievements[259] > 0) {
    multiplier *= Math.max(1, 1.01 ** Math.log2(player.hepteractCrafts.abyss.CAP))
  }
  if (G.isEvent) {
...
export const calculateAscensionScore = () => {
  ...
  for (let i = 2; i < 10; i++) {
    const exponent = ((i === 2) && player.usedCorruptions[i] >= 10)
      ? 1 + 2 * Math.min(1, player.platonicUpgrades[17]) + 0.04 * player.platonicUpgrades[17]
      : 1
    corruptionMultiplier *= G.corruptionPointMultipliers[player.usedCorruptions[i] + bonusLevel] ** exponent
      + bonusVal
  ...
  if (effectiveScore > 1e23) {
    effectiveScore = effectiveScore ** 0.5 * 1e23 ** 0.5
  }
...
export const CalcCorruptionStuff = () => {
  ...
  cubeGain = cubeGain ** bonusCubeExponent
  ...
  let hepteractGain =
    (G.challenge15Rewards.hepteractUnlocked && effectiveScore >= 1.666e17 && player.achievements[255] > 0) ? 1 : 0
  ...
  if (player.challengecompletions[10] > 0 && player.achievements[197] === 1) {
    ...
    if (player.ascensionCounter >= 10) {
      ...
      ascCount *= 1 + (player.ascensionCounter / 10 - 1) * 0.2
          * (player.achievements[189] + player.achievements[202] + player.achievements[209] + player.achievements[216]
...
export const calculateCubeQuarkMultiplier = () => {
  return (calculateSigmoid(2, player.overfluxOrbs ** 0.5, 40)
    + calculateSigmoid(1.5, player.overfluxOrbs ** 0.5, 160)
    + calculateSigmoid(1.5, player.overfluxOrbs ** 0.5, 640)
    + calculateSigmoid(1.15, +(player.highestSingularityCount >= 1) * player.overfluxOrbs ** 0.45, 2560)
    + calculateSigmoid(1.15, +(player.highestSingularityCount >= 2) * player.overfluxOrbs ** 0.4, 10000)
    + calculateSigmoid(1.25, +(player.highestSingularityCount >= 5) * player.overfluxOrbs ** 0.35, 40000)
    + calculateSigmoid(1.25, +(player.highestSingularityCount >= 10) * player.overfluxOrbs ** 0.32, 160000)
    + calculateSigmoid(1.35, +(player.highestSingularityCount >= 15) * player.overfluxOrbs ** 0.27, 640000)
    + calculateSigmoid(1.45, +(player.highestSingularityCount >= 20) * player.overfluxOrbs ** 0.24, 2e6)
    + calculateSigmoid(1.55, +(player.highestSingularityCount >= 25) * player.overfluxOrbs ** 0.21, 1e7)
    + calculateSigmoid(1.85, +(player.highestSingularityCount >= 30) * player.overfluxOrbs ** 0.18, 4e7)
    + calculateSigmoid(3, +(player.highestSingularityCount >= 35) * player.overfluxOrbs ** 0.15, 1e8)
    - 11)
...
export const calculateCubeMultFromPowder = () => {
  return (player.overfluxPowder > 10000)
    ? (1 + 1 / 16 * Math.log10(player.overfluxPowder) ** 2)
