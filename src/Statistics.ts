...
export const loadStatisticsMiscellaneous = () => {
  ...
  DOMCacheGetOrSet("sMisc2").textContent = `${format(
  ...
  DOMCacheGetOrSet("sMisc6").textContent = `${format(
  ...
  DOMCacheGetOrSet("sMisc8").textContent = `${format(
  ...
  DOMCacheGetOrSet("sMisc15").textContent = `${formatTimeShort(
    player.quarkstimer
...
export const loadStatisticsAccelerator = () => {
  DOMCacheGetOrSet("sA1").textContent = `+${format(
    G.freeUpgradeAccelerator,
    0,
    false
  )}`;
  DOMCacheGetOrSet("sA2").textContent = `+${format(
  ...
  DOMCacheGetOrSet("sA3").textContent = `+${format(
    Math.floor(Math.pow((G.rune1level * G.effectiveLevelMult) / 10, 1.1)),
    0,
    true
  )}`;
  DOMCacheGetOrSet("sA4").textContent = `x${format(
    1 + ((G.rune1level * 1) / 200) * G.effectiveLevelMult,
    3,
    true
  )}`;
  DOMCacheGetOrSet("sA5").textContent = `x${format(
    Math.pow(
      1.01,
      player.upgrades[21] +
        player.upgrades[22] +
  ...
  DOMCacheGetOrSet("sA6").textContent = `x${format(
    Math.pow(
      1.01,
  ...
  DOMCacheGetOrSet("sA7").textContent = `x${format(
    1 + (1 / 5) * player.researches[1],
    3,
    true
  )}`;
  DOMCacheGetOrSet("sA8").textContent = `x${format(
    1 +
  ...
  DOMCacheGetOrSet("sA9").textContent = `x${format(
    1 + (1 / 20) * player.researches[86],
    3,
    true
  )}`;
  DOMCacheGetOrSet("sA10").textContent = `x${format(
  ...
  DOMCacheGetOrSet("sA11").textContent = `^${format(
    Math.min(
      1,
...
export const loadStatisticsMultiplier = () => {
  DOMCacheGetOrSet("sM1").textContent = `+${format(
    G.freeUpgradeMultiplier,
    0,
    true
  )}`;
  DOMCacheGetOrSet("sM2").textContent = `+${format(
    (Math.floor(
  ...
  DOMCacheGetOrSet("sM3").textContent = `x${format(
    1 + (G.rune2level / 200) * G.effectiveLevelMult,
    3,
    true
  )}`;
  DOMCacheGetOrSet("sM4").textContent = `x${format(
    Math.pow(
      1.01,
      player.upgrades[21] +
        player.upgrades[22] +
  ...
  DOMCacheGetOrSet("sM5").textContent = `x${format(
    Math.pow(
      1.01,
  ...
  DOMCacheGetOrSet("sM6").textContent = `x${format(
    1 + (1 / 5) * player.researches[2],
    3,
    true
  )}`;
  DOMCacheGetOrSet("sM7").textContent = `x${format(
    1 +
  ...
  DOMCacheGetOrSet("sM8").textContent = `x${format(
    1 + (1 / 20) * player.researches[87],
    3,
    true
  )}`;
  DOMCacheGetOrSet("sM9").textContent = `x${format(
    calculateSigmoidExponential(
      40,
  ...
  DOMCacheGetOrSet("sM10").textContent = `x${format(
    G.cubeBonusMultiplier[2],
    3,
    true
  )}`;
  DOMCacheGetOrSet("sM11").textContent = `x${format(
    (player.currentChallenge.transcension !== 0 ||
      player.currentChallenge.reincarnation !== 0) &&
      player.upgrades[50] > 0.5
      ? 1.25
  ...
  DOMCacheGetOrSet("sM12").textContent = `^${format(
    Math.min(
      1,
      (1 + player.platonicUpgrades[6] / 30) *
        G.viscosityPower[player.usedCorruptions[2]]
...
export const loadQuarkMultiplier = () => {
  ...
  DOMCacheGetOrSet("sGQM2").textContent = `+${format(
    player.achievementPoints / 25000,
    3,
    true
  )}`; // AP
  DOMCacheGetOrSet("sGQM3").textContent = `+${format(
    player.achievements[250] > 0 ? 0.1 : 0,
    3,
    true
  )}`; // Max r8x25
  DOMCacheGetOrSet("sGQM4").textContent = `+${format(
    player.achievements[251] > 0 ? 0.1 : 0,
    3,
    true
  )}`; // Max w5x10
  DOMCacheGetOrSet("sGQM5").textContent = `+${format(
    player.platonicUpgrades[5] > 0 ? 0.2 : 0,
    3,
    true
  )}`; // ALPHA
  DOMCacheGetOrSet("sGQM6").textContent = `+${format(
    player.platonicUpgrades[10] > 0 ? 0.25 : 0,
    3,
    true
  )}`; // BETA
  DOMCacheGetOrSet("sGQM7").textContent = `+${format(
    player.platonicUpgrades[15] > 0 ? 0.3 : 0,
    3,
    true
  )}`; // OMEGA
  DOMCacheGetOrSet("sGQM8").textContent = `+${format(
    G.challenge15Rewards.quarks - 1,
    3,
    true
  )}`; // Challenge 15 Reward
  DOMCacheGetOrSet("sGQM9").textContent = `x${format(
    player.worlds.applyBonus(1 / calculateQuarkMultiplier()),
    3,
    true
  )}`; // Patreon Bonus
  DOMCacheGetOrSet("sGQM10").textContent = `x${format(
    G.isEvent
      ? 1 +
  ...
  DOMCacheGetOrSet("sGQM11").textContent = `x${format(
    1.1 + (0.15 / 75) * calculateEffectiveIALevel(),
    3,
    true
  )}`; // IA Rune
  DOMCacheGetOrSet("sGQM12").textContent = `x${format(
  ...
  DOMCacheGetOrSet("sGQM13").textContent = `x${format(
    calculateQuarkMultFromPowder(),
    3,
    true
  )}`; // Powder
  DOMCacheGetOrSet("sGQM14").textContent = `x${format(
    1 + player.achievements[266] * Math.min(0.1, player.ascensionCount / 1e16),
    3,
    true
  )}`; // Achievement 266 [Max: 10% at 1Qa Ascensions]
  DOMCacheGetOrSet("sGQM15").textContent = `x${format(
    1 + player.singularityCount / 10,
    3,
    true
  )}`; // Singularity
  DOMCacheGetOrSet("sGQM16").textContent = `x${format(
    calculateSingularityQuarkMilestoneMultiplier(),
    3,
    true
  )}`; // Singularity Milestones
  DOMCacheGetOrSet("sGQM17").textContent = `x${format(
    1 + (0.1 * player.cubeUpgrades[53]) / 100,
    3,
    true
  )}`; // Cube Upgrade 6x3 (Cx3)
  DOMCacheGetOrSet("sGQM18").textContent = `x${format(
    1 +
  ...
  DOMCacheGetOrSet("sGQM19").textContent = `x${format(
    1 +
  ...
  DOMCacheGetOrSet("sGQM20").textContent = `x${format(
    1 + 0.25 * +player.octeractUpgrades.octeractStarter.getEffect().bonus,
    3,
    true
  )}`;
  DOMCacheGetOrSet("sGQM21").textContent = `x${format(
    +player.octeractUpgrades.octeractQuarkGain.getEffect().bonus,
    3,
    true
  )}`;
  DOMCacheGetOrSet("sGQM22").textContent = `x${format(
    calculateTotalOcteractQuarkBonus(),
    3,
    true
  )}`;
  DOMCacheGetOrSet("sGQM23").textContent = `x${format(
    1 + +player.singularityUpgrades.singQuarkImprover1.getEffect().bonus,
    3,
    true
  )}`;
  DOMCacheGetOrSet("sGQM24").textContent = `x${format(
    1 +
      (1 / 10000) *
  ...
  DOMCacheGetOrSet("sGQM25").textContent = `x${format(
    calculateAmbrosiaQuarkMult(),
    2,
    true
  )}`;
  DOMCacheGetOrSet("sGQM26").textContent = `x${format(
    +player.blueberryUpgrades.ambrosiaTutorial.bonus.quarks,
    2,
    true
  )}`;
  DOMCacheGetOrSet("sGQM27").textContent = `x${format(
    +player.blueberryUpgrades.ambrosiaQuarks1.bonus.quarks,
    2,
    true
  )}`;
  DOMCacheGetOrSet("sGQM28").textContent = `x${format(
    +player.blueberryUpgrades.ambrosiaCubeQuark1.bonus.quarks,
    2,
    true
  )}`;
  DOMCacheGetOrSet("sGQM29").textContent = `x${format(
    +player.blueberryUpgrades.ambrosiaLuckQuark1.bonus.quarks,
    2,
    true
  )}`;
  DOMCacheGetOrSet("sGQM30").textContent = `x${format(
    +player.blueberryUpgrades.ambrosiaQuarks2.bonus.quarks,
    2,
    true
  )}`;
  DOMCacheGetOrSet("sGQMT").textContent = `x${format(
...
export const loadGlobalSpeedMultiplier = () => {
  ...
  for (let i = 0; i < preDRlist.length; i++) {
    DOMCacheGetOrSet(`sGSMa${i + 1}`).textContent = `x${format(
      preDRlist[i],
      3,
      true
    )}`;
  ...
  for (let i = 0; i < drList.length; i++) {
    DOMCacheGetOrSet(`sGSMb${i + 1}`).textContent = `x${format(
      drList[i],
      3,
      true
    )}`;
  ...
  for (let i = 0; i < postDRlist.length; i++) {
    DOMCacheGetOrSet(`sGSMc${i + 1}`).textContent = `x${format(
      postDRlist[i],
      3,
      true
    )}`;
...
export const loadStatisticsCubeMultipliers = () => {
  ...
  for (let i = 0; i < arr0.length; i++) {
    ...
    DOMCacheGetOrSet(`sGCM${i + 1}`).textContent = `x${format(
      arr0[i],
      map0[i + 1].acc,
      true
    )}`;
  ...
  DOMCacheGetOrSet("sGCMT").textContent = `x${format(
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    DOMCacheGetOrSet(`sCM${i + 1}`).textContent = `x${format(
      arr[i],
      map[i + 1].acc,
      true
    )}`;
  ...
  // PLAT
  DOMCacheGetOrSet("sCMT").textContent = `x${format(
  ...
  for (let i = 0; i < arr2.length; i++) {
    ...
    DOMCacheGetOrSet(`sTeM${i + 1}`).textContent = `x${format(
      arr2[i],
      map2[i + 1].acc,
      true
    )}`;
  ...
  DOMCacheGetOrSet("sTeMT").textContent = `x${format(
  ...
  for (let i = 0; i < arr3.length; i++) {
    ...
    DOMCacheGetOrSet(`sHyM${i + 1}`).textContent = `x${format(
      arr3[i],
      map3[i + 1].acc,
      true
    )}`;
  ...
  DOMCacheGetOrSet("sHyMT").textContent = `x${format(
  ...
  for (let i = 0; i < arr4.length; i++) {
    ...
    DOMCacheGetOrSet(`sPlM${i + 1}`).textContent = `x${format(
      arr4[i],
      map4[i + 1].acc,
      true
    )}`;
  ...
  DOMCacheGetOrSet("sPlMT").textContent = `x${format(
  ...
  for (let i = 0; i < arr5.length; i++) {
    ...
    DOMCacheGetOrSet(`sHeM${i + 1}`).textContent = `x${format(
      arr5[i],
      map5[i + 1].acc,
      true
    )}`;
  ...
  DOMCacheGetOrSet("sHeMT").textContent = `x${format(
  ...
  for (let i = 0; i < octMults.list.length; i++) {
    ...
    DOMCacheGetOrSet(`sOcM${i + 1}`).textContent = `x${format(
      octMults.list[i],
      map6[i + 1].acc,
      true
    )}`;
...
export const loadStatisticsOfferingMultipliers = () => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    DOMCacheGetOrSet(`sOff${i + 1}`).textContent = `x${format(
      arr[i],
      map[i + 1].acc,
      true
    )}`;
  }
  DOMCacheGetOrSet("sOffT").textContent = `x${format(
...
export const loadPowderMultiplier = () => {
  ...
  for (let i = 0; i < arr0.length; i++) {
    ...
    statGCMi.childNodes[0].textContent = map0[i + 1].desc;
    DOMCacheGetOrSet(`sPoM${i + 1}`).textContent = `x${format(
      arr0[i],
      map0[i + 1].acc,
      true
    )}`;
  ...
  DOMCacheGetOrSet("sPoMT").textContent = `x${format(
...
export const loadStatisticsAscensionSpeedMultipliers = () => {
  ...
  for (let i = 0; i < arr.list.length; i++) {
    ...
    DOMCacheGetOrSet(`sASM${i + 1}`).textContent = `x${format(
      arr.list[i],
      map7[i + 1].acc,
      true
    )}`;
...
export const loadStatisticsGoldenQuarkMultipliers = () => {
  ...
  for (let i = 0; i < arr.list.length; i++) {
    ...
    DOMCacheGetOrSet(`sGQMS${i + 1}`).textContent = `x${format(
      arr.list[i],
      map[i + 1].acc,
      true
    )}`;
...
export const loadAddCodeModifiersAndEffects = () => {
  ...
  for (let i = 0; i < intervalStats.list.length; i++) {
    ...
    if (i === 0) {
      DOMCacheGetOrSet(`s+time${i + 1}`).textContent = `${format(
        intervalStats.list[i],
        intervalMap[i + 1].acc,
        true
      )} sec`;
    } else {
      DOMCacheGetOrSet(`s+time${i + 1}`).textContent = `x${format(
        intervalStats.list[i],
        intervalMap[i + 1].acc,
        true
      )}`;
  ...
  DOMCacheGetOrSet("s+timeT").textContent = `${format(
    intervalStats.time / 1000,
    1
  )} sec`;
  if (availableCount !== capacityStats.total) {
    DOMCacheGetOrSet("s+next").textContent = `+1 in ${format(
      timeToNext,
      1
  ...
  for (let i = 0; i < capacityStats.list.length; i++) {
    ...
    DOMCacheGetOrSet(`s+cap${i + 1}`).textContent = `${prefix}${format(
  ...
  DOMCacheGetOrSet("s+capT").textContent = `${format(
    availableCount,
    0
  ...
  if (Math.abs(addEffectStats.maxQuarks - addEffectStats.minQuarks) >= 0.5) {
    ...
    DOMCacheGetOrSet("s+eff1").textContent = `+${format(
  ...
  DOMCacheGetOrSet("stat+eff2").childNodes[0].textContent =
    "PL-AT X - bonus ascension time: ";
  DOMCacheGetOrSet("s+eff2").textContent = `+${format(
    addEffectStats.ascensionTimer,
    2
  )} sec`;

  DOMCacheGetOrSet("stat+eff3").childNodes[0].textContent =
    "PL-AT Γ - bonus GQ export time: ";
  DOMCacheGetOrSet("s+eff3").textContent = `+${format(
    addEffectStats.gqTimer,
    2
  )} sec`; // does it need a / 1000?

  DOMCacheGetOrSet("stat+eff4").childNodes[0].textContent =
    "PL-AT _ - bonus octeract time: ";
  DOMCacheGetOrSet("s+eff4").textContent = `+${format(
...
export const loadStatisticsAmbrosiaLuck = () => {
  ...
  for (let i = 0; i < arr.length - 1; i++) {
    ...
    DOMCacheGetOrSet(`sALuckM${i + 1}`).textContent = `+${format(
      arr[i],
      map[i + 1].acc,
      true
    )}`;
...
export const loadStatisticsAmbrosiaGeneration = () => {
  ...
  for (let i = 0; i < arr.length - 1; i++) {
    ...
    DOMCacheGetOrSet(`sAGenM${i + 1}`).textContent = `x${format(
      arr[i],
      map[i + 1].acc,
      true
    )}`;
...
export const c15RewardUpdate = () => {
  ...
  if (e >= exponentRequirements[0]) {
    ...
    G.challenge15Rewards[keys[0]] =
      1 + ((1 / 50) * Math.log(e / 175)) / Math.log(2);
  }
  if (e >= exponentRequirements[1]) {
    // Ascension Count [1500]
    G.challenge15Rewards[keys[1]] =
      1 + ((1 / 20) * Math.log(e / 375)) / Math.log(2);
  }
  if (e >= exponentRequirements[2]) {
    // Coin Exponent [3000]
    G.challenge15Rewards[keys[2]] =
      1 + ((1 / 150) * Math.log(e / 750)) / Math.log(2);
  }
  ...
  if (e >= exponentRequirements[6]) {
    ...
    G.challenge15Rewards[keys[6]] =
      1 + ((1 / 20) * Math.log(e / 2.5e3)) / Math.log(2);
  }
  if (e >= exponentRequirements[7]) {
    // Multiplier Boost (Uncorruptable) [10000]
    G.challenge15Rewards[keys[7]] =
      1 + ((1 / 20) * Math.log(e / 2.5e3)) / Math.log(2);
  }
  ...
  if (e >= exponentRequirements[9]) {
    ...
    G.challenge15Rewards[keys[9]] =
      1 + ((1 / 33) * Math.log(e / 1e4)) / Math.log(2);
  }
  if (e >= exponentRequirements[10]) {
    // All Cube Types II [60000]
    G.challenge15Rewards[keys[10]] =
      1 + ((1 / 100) * Math.log(e / 1.5e4)) / Math.log(2);
  }
  ...
  if (e >= exponentRequirements[14]) {
    ...
    G.challenge15Rewards[keys[14]] =
      1 + ((1 / 20) * Math.log(e / 1.5e5)) / Math.log(2);
  }
  if (e >= exponentRequirements[15]) {
    // All Cube Types III [1m]
    G.challenge15Rewards[keys[15]] =
      1 + ((1 / 150) * Math.log(e / 2.5e5)) / Math.log(2);
  }
  if (e >= exponentRequirements[16]) {
    // Talisman Effectiveness [3m]
    G.challenge15Rewards[keys[16]] =
      1 + ((1 / 20) * Math.log(e / 7.5e5)) / Math.log(2);
  }
  if (e >= exponentRequirements[17]) {
    // Global Speed [10m]
    G.challenge15Rewards[keys[17]] =
      1 + ((1 / 20) * Math.log(e / 2.5e6)) / Math.log(2);
  }
  ...
  if (e >= exponentRequirements[20]) {
    ...
    G.challenge15Rewards[keys[20]] =
      1 + ((1 / 200) * Math.log(e / 1.25e8)) / Math.log(2);
  }
  ...
  if (e >= exponentRequirements[22]) {
    ...
    if (e >= 1e20) {
      G.challenge15Rewards[keys[22]] =
        1 + (1 / 4) * Math.pow(e / 1e10, 1 / 8) * Math.pow(1e10, 1 / 8);
    }
  }
  if (e >= exponentRequirements[23]) {
    // Quark Gain [100b]
    G.challenge15Rewards[keys[23]] =
      1 + ((1 / 100) * Math.log((e * 32) / 1e11)) / Math.log(2);
  }
  ...
  if (e >= exponentRequirements[26]) {
    ...
    G.challenge15Rewards[keys[25]] =
      1 + (1 / 300) * Math.log2(e / (4e15 / 1024));
  }
  ...
  if (e >= exponentRequirements[34]) {
    // Ascension Speed
    G.challenge15Rewards.ascensionSpeed =
      1 + 5 / 100 + (2 * Math.log2(e / 1.5e18)) / 100;
  }
...
const updateDisplayC15Rewards = () => {
  ...
  for (let i = 0; i < exponentRequirements.length; i++) {
    ...
    DOMCacheGetOrSet(`c15Reward${i + 1}`).style.display =
      player.challenge15Exponent >= exponentRequirements[i] ? "block" : "none";
    DOMCacheGetOrSet("c15RewardList").textContent =
      typeof keepExponent === "string"
        ? "You have unlocked all reward types from Challenge 15!"
        : `Next reward type requires ${format(
            keepExponent,
            0,
            true
          )} exponent.`;
