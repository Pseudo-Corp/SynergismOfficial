...
export const loadStatisticsAccelerator = () => {
  ...
  DOMCacheGetOrSet('sA3').textContent = `+${
    format(Math.floor((G.rune1level * G.effectiveLevelMult / 10) ** 1.1), 0, true)
  }`
  ...
  DOMCacheGetOrSet('sA6').textContent = `x${
    format(1.01 ** (player.achievements[60] + player.achievements[61] + player.achievements[62]), 3, true)
  }`
...
export const loadStatisticsMultiplier = () => {
  ...
  DOMCacheGetOrSet('sM5').textContent = `x${
    format(1.01 ** (player.achievements[57] + player.achievements[58] + player.achievements[59]), 3, true)
  }`
...
export const c15RewardUpdate = () => {
  ...
  if (e >= exponentRequirements[3]) {
    // Taxes [5000]
    G.challenge15Rewards[keys[3]] = 0.98 ** (Math.log(e / 1.25e3) / Math.log(2))
  }
  if (e >= exponentRequirements[4]) {
    // Obtainium [7500]
    G.challenge15Rewards[keys[4]] = 1 + 1 / 5 * (e / 7.5e3) ** 0.75
  }
  if (e >= exponentRequirements[5]) {
    // Offerings [7500]
    G.challenge15Rewards[keys[5]] = 1 + 1 / 5 * (e / 7.5e3) ** 0.75
  }
  ...
  if (e >= exponentRequirements[8]) {
    // Rune EXP [20000]
    G.challenge15Rewards[keys[8]] = 1 + (e / 2e4) ** 1.5
  }
  ...
  if (e >= exponentRequirements[11]) {
    ...
    G.challenge15Rewards[keys[11]] = 0.98 ** (Math.log(e / 2.5e4) / Math.log(2))
  }
  if (e >= exponentRequirements[12]) {
    // Chal 6-10 Scaling [100000]
    G.challenge15Rewards[keys[12]] = 0.98 ** (Math.log(e / 2.5e4) / Math.log(2))
  }
  if (e >= exponentRequirements[13]) {
    // Ant Speed [200k]
    G.challenge15Rewards[keys[13]] = (1 + Math.log(e / 2e5) / Math.log(2)) ** 4
  }
  ...
  if (e >= exponentRequirements[18]) {
    ...
    G.challenge15Rewards[keys[18]] = 1 + 1 / 5 * (e / 3e7) ** (1 / 4)
  }
  if (e >= exponentRequirements[19]) {
    // Tesseract Building Speed [100m]
    G.challenge15Rewards[keys[19]] = 1 + 1 / 5 * (e / 1e8) ** (2 / 3)
  }
  ...
  if (e >= exponentRequirements[21]) {
    ...
    G.challenge15Rewards[keys[21]] = 1 + 1 / 5 * (e / 2e9) ** (1 / 4)
  }
  if (e >= exponentRequirements[22]) {
    // Ascension Score [10b]
    G.challenge15Rewards[keys[22]] = 1 + 1 / 4 * (e / 1e10) ** (1 / 4)
    if (e >= 1e20) {
      G.challenge15Rewards[keys[22]] = 1 + 1 / 4 * (e / 1e10) ** (1 / 8) * 1e10 ** (1 / 8)
    }
  }
  ...
  if (e >= exponentRequirements[33]) {
    // FREE Daily Orbs
    G.challenge15Rewards.freeOrbs = Math.floor(200 * (e / 2e17) ** 0.5)
  }
