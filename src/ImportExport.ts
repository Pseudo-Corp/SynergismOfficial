...
export const promocodes = async (input: string | null, amount?: number) => {
  ...
  if (
    ...
    if (player.highestSingularityCount > 0) {
      player.singularityUpgrades.goldenQuarks1.freeLevels +=
        1 + Math.floor(player.highestSingularityCount / 25);
      player.singularityUpgrades.goldenQuarks2.freeLevels +=
        1 + Math.floor(player.highestSingularityCount / 25);
      player.singularityUpgrades.goldenQuarks3.freeLevels +=
        1 + Math.floor(player.highestSingularityCount / 25);
  ...
  if (input === "synergism2021" && !player.codes.get(1)) {
    ...
    if (player.highestSingularityCount > 0) {
      ...
      rolls +=
        +player.octeractUpgrades.octeractImprovedDaily3.getEffect().bonus;
      rolls *=
        +player.octeractUpgrades.octeractImprovedDaily2.getEffect().bonus;
      rolls *=
        1 +
      ...
      for (let i = 0; i < rolls; i++) {
        ...
        for (const key of keys) {
          if (upgradeDistribution[key].pdf(num)) {
            player.singularityUpgrades[key].freeLevels +=
              upgradeDistribution[key].value;
            freeLevels[key]
      ...
      if (player.highestSingularityCount >= 200) {
        player.octeractUpgrades.octeractGain.freeLevels +=
          player.octeractUpgrades.octeractGain.level / 100;
        freeLevels.octeractGain =
          player.octeractUpgrades.octeractGain.level / 100;
      }

      if (player.highestSingularityCount >= 205) {
        player.octeractUpgrades.octeractGain2.freeLevels +=
          player.octeractUpgrades.octeractGain2.level / 100;
        freeLevels.octeractGain2 =
          player.octeractUpgrades.octeractGain2.level / 100;
      }
    ...
    const realAttemptsUsed =
      toUse > 0 ? Math.min(availableUses, toUse) : availableUses + toUse;
    ...
    const gqTimerText =
      player.shopUpgrades.calculator5 > 0
    ...
    const octeractTimeText =
      player.shopUpgrades.calculator6 > 0
    ...
    if (player.shopUpgrades.calculator === shopData.calculator.maxLevel) {
      ...
      if (player.highestSingularityCount >= 150) {
        player.singularityUpgrades.goldenQuarks1.freeLevels +=
          0.01 * realAttemptsUsed;
        player.singularityUpgrades.goldenQuarks3.freeLevels +=
          0.05 * realAttemptsUsed;
      }
    ...
    const amount =
      1 + (window.crypto.getRandomValues(new Uint16Array(1))[0] % 16); // [1, 16]
    ...
    if (playerConfirmed) {
      ...
      if (diff <= 2500 + 125 * player.cubeUpgrades[61]) {
        ...
        if (actualQuarkAward > 66666) {
          actualQuarkAward =
            Math.pow(actualQuarkAward, 0.35) * Math.pow(66666, 0.65);
        }
...
export const addCodeTimeToNextUse = (): number => {
  const timeToFirst =
    Math.floor(addCodeInterval().time + player.rngCode - Date.now()) / 1000;
  ...
  if (timeToFirst > 0) {
    ...
    const remainder =
      addTimerElapsedTime - addCodeInterval().time * addCodeAvailableUses();
...
export const addCodeBonuses = () => {
  ...
  const ascensionTimer =
    (60 * player.shopUpgrades.calculator3 * ascMult) / perkRewardDivisor;
...
const dailyCodeFormatFreeLevelMessage = (
  ...
  const upgradeNiceName =
    upgradeKey in singularityData
