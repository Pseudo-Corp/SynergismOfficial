...
export const displayRuneInformation = (i: number, updatelevelup = true) => {
    ...
    if (i === 1) {
        ...
        document.getElementById("runeshowpower1").childNodes[0].textContent = "Speed Rune Bonus: " + "+" + format(Math.floor(Math.pow(G['rune1level'] * m / 4, 1.25))) + " Accelerators, +" + format((G['rune1level'] / 4 * m), 2, true) + "% Accelerators, +" + format(Math.floor(G['rune1level'] / 20 * m)) + " Accelerator Boosts."
    }
    if (i === 2) {
        if (updatelevelup) {
        ...
        document.getElementById("runeshowpower2").childNodes[0].textContent = "Duplication Rune Bonus: " + "+" + format(Math.floor(G['rune2level'] * m / 10) * Math.floor(1 + G['rune2level'] * m / 10) / 2) + " Multipliers, +" + format(m * G['rune2level'] / 4, 1, true) + "% Multipliers, -" + (99.9 * (1 - Math.pow(6, -(G['rune2level'] * m) / 1000))).toPrecision(4) + "% Tax Growth."
    }
    if (i === 3) {
        if (updatelevelup) {
        ...
        document.getElementById("runeshowpower3").childNodes[0].textContent = "Prism Rune Bonus: " + "All Crystal Producer production multiplied by " + format(Decimal.pow(G['rune3level'] * m / 2, 2).times(Decimal.pow(2, G['rune3level'] * m / 2 - 8)).add(1), 3) + ", gain +" + format(Math.floor(G['rune3level'] / 16 * m)) + " free crystal levels."
    }
    if (i === 4) {
        if (updatelevelup) {
        ...
        document.getElementById("runeshowpower4").childNodes[0].textContent = "Thrift Rune Bonus: " + "Delay all producer cost increases by " + (G['rune4level'] / 8 * m).toPrecision(3) + "%. Offering recycle chance: +" + Math.min(25, G['rune4level'] / 16) + "%. -" + (99 * (1 - Math.pow(4, Math.min(0, (400 - G['rune4level']) / 1100)))).toPrecision(4) + "% Tax Growth"
    }
    if (i === 5) {
        if (updatelevelup) {
