  ...
  if (i === G.challengefocus) {
    ...
    switch (i) {
      ...
      case 5: {
        ...
        current2 = format(10 ** CalcECC('transcend', player.challengecompletions[5]))
        break
      }
      case 6: {
        current1 = format(0.965 ** CalcECC('reincarnation', player.challengecompletions[6]), 3, true)
      ...
      case 9: {
        ...
        current2 = format(1.1 ** CalcECC('reincarnation', player.challengecompletions[9]), 2, true)
      ...
      case 13: {
        current1 = format(100 - 100 * 0.966 ** CalcECC('ascension', player.challengecompletions[13]), 3, true)
...
export const calculateChallengeRequirementMultiplier = (
  ...
  switch (type) {
    case 'transcend':
      ...
      ;(completions >= 75)
        ? requirementMultiplier *= (1 + completions) ** 12 / 75 ** 8
        : requirementMultiplier *= (1 + completions) ** 2

      if (completions >= 1000) {
        requirementMultiplier *= 10 * (completions / 1000) ** 3
      }
      if (completions >= 9000) {
    ...
    case 'reincarnation':
      if (completions >= 100 && (special === 9 || special === 10)) {
        requirementMultiplier *= 1.05 ** ((completions - 100) * (1 + (completions - 100) / 20))
      }
      if (completions >= 90) {
        if (special === 6) {
      ...
      if (completions >= 25) {
        requirementMultiplier *= (1 + completions) ** 5 / 625
      }
      if (completions < 25) {
        requirementMultiplier *= Math.min((1 + completions) ** 2, 1.3797 ** completions)
      }
    ...
    case 'ascension':
      if (special !== 15) {
        ...
        requirementMultiplier *= 1000 ** completions
      }
      return requirementMultiplier
  }
}
...
export const challengeRequirement = (challenge: number, completion: number, special = 0) => {
  ...
  if (challenge <= 5) {
    ...
    return Decimal.pow(
      10,
      1 * 10 ** 30 * calculateChallengeRequirementMultiplier('ascension', completion, special)
    )
  } else {
    return 0
  }
