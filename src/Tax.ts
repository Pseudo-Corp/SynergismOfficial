...
export const calculatetax = () => {
    ...
    if (player.currentChallenge.ascension === 13) {
        ...
        exp *= Math.pow(1.05, Math.max(0, sumContents(player.challengecompletions) - player.challengecompletions[11] - player.challengecompletions[12] - player.challengecompletions[13] - player.challengecompletions[14] - player.challengecompletions[15] - 3 * player.cubeUpgrades[49] - 4 * ((player.singularityCount >= 15)?1:0) - 1 * ((player.singularityCount >= 20)?1:0)));
    }
