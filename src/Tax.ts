...
export const calculatetax = () => {
  ...
  if (player.currentChallenge.reincarnation === 6) {
    exp = 3 * (1 + player.challengecompletions[6] / 25) ** 2
  }
  ...
  exponent *= 0.965 ** CalcECC('reincarnation', player.challengecompletions[6])
  exponent *= 0.001 + .999 * (6 ** (-(G.rune2level * G.effectiveLevelMult) / 1000))
  exponent *= 0.01 + .99 * (4 ** Math.min(0, (400 - G.rune4level) / 1100))
  ...
  exponent *= 0.005 + 0.995 * 0.99 ** (player.antUpgrades[2]! + G.bonusant3)
  exponent *= 1
    / (
  ...
  exponent *= 0.98 ** (3 / 5 * Math.log(1 + player.rareFragments) / Math.log(10) * player.researches[159])
  exponent *= 0.966 ** CalcECC('ascension', player.challengecompletions[13])
