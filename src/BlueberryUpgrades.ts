...
export interface IBlueberryData
  extends Omit<IUpgradeData, "name" | "description" | "effect"> {
...
export class BlueberryUpgrade extends DynamicUpgrade {
  ...
  toString(): string {
    ...
    const maxLevel =
      this.maxLevel === -1 ? "" : `/${format(this.maxLevel, 0, true)}`;
    ...
    const affordableInfo = isMaxLevel
      ...
      : isAffordable
      ? `<span style="color: var(--green-text-color)"> ${i18next.t(
          "general.affordable"
        )}</span>`
      : `<span style="color: yellow"> ${i18next.t(
    ...
    if (this.preRequisites !== undefined) {
      for (const [prereq, val] of Object.entries(this.preRequisites)) {
        ...
        const color =
          player.blueberryUpgrades[k].level >= val ? "green" : "red";
        const met =
    ...
    return `<span style="color: gold">${this.name}</span>
      ...
      true
                ...
                ${i18next.t("general.spent")} ${i18next.t(
      "ambrosia.ambrosia"
    )}: <span style="color:orange">${format(
  ...
  updateUpgradeHTML(): void {
    DOMCacheGetOrSet("singularityAmbrosiaMultiline").innerHTML =
      this.toString();
...
export const createBlueberryTree = async (modules: BlueberryOpt) => {
  ...
  for (const [key, val] of Object.entries(actualModules)) {
    const k = key as keyof Player["blueberryUpgrades"];
    const { costFormula, costPerLevel, blueberryCost } =
      player.blueberryUpgrades[k];
