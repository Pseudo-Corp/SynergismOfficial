...
export const blueberryUpgradeData: Record<keyof Player['blueberryUpgrades'], IBlueberryData> = {
  ambrosiaTutorial: {
    ...
    costFormula: (level: number, baseCost: number): number => {
      return baseCost * ((level + 1) ** 2 - level ** 2)
    },
    rewards: (n: number) => {
  ...
  ambrosiaQuarks1: {
    ...
    blueberryCost: 0,
    costFormula: (level: number, baseCost: number): number => {
      return baseCost * ((level + 1) ** 3 - level ** 3)
    },
    rewards: (n: number) => {
      const quarkAmount = 1 + 0.01 * n
      return {
  ...
  ambrosiaCubes1: {
    ...
    rewards: (n: number) => {
      const cubeAmount = (1 + 0.05 * n) * 1.1 ** Math.floor(n / 10)
      return {
        cubes: cubeAmount,
        desc: String(
  ...
  ambrosiaQuarkCube1: {
    ...
    rewards: (n: number) => {
      ...
      const val = 1 + baseVal * Math.floor((Math.log10(Number(player.worlds) + 1) + 1) ** 2)
      return {
        cubes: val,
  ...
  ambrosiaLuckQuark1: {
    ...
    rewards: (n: number) => {
      ...
      const effectiveLuck = Math.min(
        ...
        1000 ** 0.5 * player.caches.ambrosiaLuck.totalVal ** 0.5
      )
  ...
  ambrosiaQuarkLuck1: {
    ...
    rewards: (n: number) => {
      const baseVal = 0.02 * n
      const val = baseVal * Math.floor((Math.log10(Number(player.worlds) + 1) + 1) ** 2)
      return {
        ambrosiaLuck: val,
