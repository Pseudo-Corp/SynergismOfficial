...
export const updateSingularityPenalties = (): void => {
  ...
  const str = `${getSingularityOridnalText(singularityCount)}<br>${i18next.t(
...
export interface ISingularityData
  extends Omit<IUpgradeData, "name" | "description"> {
...
export class SingularityUpgrade extends DynamicUpgrade {
  ...
  toString(): string {
    ...
    const maxLevel =
      this.maxLevel === -1 ? "" : `/${format(this.computeMaxLevel(), 0, true)}`;
    const color = this.computeMaxLevel() === this.level ? "plum" : "white";
    const minReqColor =
      player.highestSingularityCount < this.minimumSingularity
        ? "var(--crimson-text-color)"
        : "var(--green-text-color)";
    const minimumSingularity =
      this.minimumSingularity > 0
        ? `${i18next.t("general.minimum")} Singularity: ${
            this.minimumSingularity
          }`
        : i18next.t("singularity.toString.noMinimum");

    let freeLevelInfo =
      this.freeLevels > 0
        ? `<span style="color: orange"> [+${format(
            this.freeLevels,
            2,
            true
          )}]</span>`
    ...
    if (this.freeLevels > this.level) {
      freeLevelInfo = `${freeLevelInfo}<span style="color: var(--maroon-text-color)"> ${i18next.t(
        "general.softCapped"
      )}</span>`;
    }
    ...
    return `<span style="color: gold">${this.name}</span>
                ...
                <span style="color: ${color}"> ${i18next.t(
      "general.level"
    )} ${format(this.level, 0, true)}${maxLevel}${freeLevelInfo}</span>
                <span style="color: gold">${this.getEffect().desc}</span>
                ${i18next.t("singularity.toString.costNextLevel")}: ${format(
      costNextLevel,
      0,
      true
    )} Golden Quarks.
                ${i18next.t("general.spent")} Quarks: ${format(
  ...
  getCostTNL(): number {
    ...
    costMultiplier *=
      this.maxLevel === -1 && this.level >= 100 ? this.level / 50 : 1;
    costMultiplier *=
      this.maxLevel === -1 && this.level >= 400 ? this.level / 100 : 1;
  ...
  public async buyLevel(event: MouseEvent): Promise<void> {
    ...
    while (maxPurchasable > 0) {
      ...
      if (this.name === player.singularityUpgrades.singCitadel2.name) {
        player.singularityUpgrades.singCitadel.freeLevels =
          player.singularityUpgrades.singCitadel2.level;
      }
  ...
  public actualTotalLevels(): number {
    ...
    if (player.octeractUpgrades.octeractImprovedFree.getEffect().bonus) {
      let exponent = 0.6;
      exponent +=
        +player.octeractUpgrades.octeractImprovedFree2.getEffect().bonus;
      exponent +=
        +player.octeractUpgrades.octeractImprovedFree3.getEffect().bonus;
      exponent +=
        +player.octeractUpgrades.octeractImprovedFree4.getEffect().bonus;
  ...
  corruptionFourteen: {
    ...
    effect: (n: number) => {
      return {
        ...
        get desc() {
          return i18next.t(
            `singularity.data.corruptionFourteen.effect${
              n > 0 ? "Have" : "HaveNot"
            }`,
            {
              m: n > 0 ? ":)" : ":(",
  ...
  corruptionFifteen: {
    ...
    effect: (n: number) => {
      return {
        ...
        get desc() {
          return i18next.t(
            `singularity.data.corruptionFifteen.effect${
              n > 0 ? "Have" : "HaveNot"
            }`,
            {
              m: n > 0 ? ":)" : ":(",
  ...
  octeractUnlock: {
    ...
    effect: (n: number) => {
      return {
        ...
        get desc() {
          return i18next.t(
            `singularity.data.octeractUnlock.effect${
              n > 0 ? "Have" : "HaveNot"
            }`
          );
        },
  ...
  intermediatePack: {
    ...
    effect: (n: number) => {
      return {
        ...
        get desc() {
          return i18next.t(
            `singularity.data.intermediatePack.effect${
              n > 0 ? "Have" : "HaveNot"
            }`
          );
        },
...
export const getGoldenQuarkCost = (): {
  ...
  costReduction *=
    +player.octeractUpgrades.octeractGQCostReduce.getEffect().bonus;
  costReduction *=
