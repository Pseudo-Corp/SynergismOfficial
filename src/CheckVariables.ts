...
export const checkVariablesOnLoad = (data: PlayerSave) => {
  ...
  if (player.antSacrificeTimerReal === undefined) {
    player.antSacrificeTimerReal =
      player.antSacrificeTimer / calculateTimeAcceleration().mult;
  }
  ...
  if (data.singularityUpgrades != null) {
    for (const item in blankSave.singularityUpgrades) {
      ...
      if (data.singularityUpgrades[k]) {
        const { level, goldenQuarksInvested, toggleBuy, freeLevels } =
          data.singularityUpgrades[k];
  ...
  if (data.octeractUpgrades != null) {
    ...
    for (const item in blankSave.octeractUpgrades) {
      ...
      if (data.octeractUpgrades[k]) {
        const { level, octeractsInvested, toggleBuy, freeLevels } =
          data.octeractUpgrades[k];
        updatedData = {
    ...
    for (const item in blankSave.singularityChallenges) {
      ...
      if (data.singularityChallenges[k]) {
        ...
        if (
          ...
          if (comps > 0) {
            data.singularityChallenges[k].highestSingularityCompleted =
              4 * comps - 3;
          }
        }

        const { completions, highestSingularityCompleted, enabled } =
          data.singularityChallenges[k];
        updatedData = {
