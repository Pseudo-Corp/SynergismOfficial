...
export const revealStuff = () => {
  ...
  for (const ex of Array.from(example32)) { // Ability to use and gain hepteracts //
    ex.style.display = player.challenge15Exponent >= G.challenge15Rewards.hepteractsUnlocked.requirement ? 'block' : 'none'
  ...
  player.highestSingularityCount > 0 && player.challenge15Exponent >= G.challenge15Rewards.hepteractsUnlocked.requirement
  ...
  const automationUnlocks: Record<string, boolean> = {
    ...
    toggle35: player.challenge15Exponent >= G.challenge15Rewards.hepteractsUnlocked.requirement && player.highestSingularityCount > 0, // Hepteracts - Notifications
