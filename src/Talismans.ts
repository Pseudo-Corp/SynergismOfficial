...
export const showTalismanPrices = (i: number) => {
  ...
  a.textContent = format(m * Math.max(0, Math.floor(1 + 1 / 8 * player.talismanLevels[i] ** 3)))
  b.textContent = format(m * Math.max(0, Math.floor(1 + 1 / 32 * (player.talismanLevels[i] - 30) ** 3)))
  c.textContent = format(m * Math.max(0, Math.floor(1 + 1 / 384 * (player.talismanLevels[i] - 60) ** 3)))
  d.textContent = format(m * Math.max(0, Math.floor(1 + 1 / 500 * (player.talismanLevels[i] - 90) ** 3)))
  e.textContent = format(m * Math.max(0, Math.floor(1 + 1 / 375 * (player.talismanLevels[i] - 120) ** 3)))
  f.textContent = format(m * Math.max(0, Math.floor(1 + 1 / 192 * (player.talismanLevels[i] - 150) ** 3)))
  g.textContent = format(m * Math.max(0, Math.floor(1 + 1 / 1280 * (player.talismanLevels[i] - 150) ** 3)))
}
...
export const buyTalismanLevels = (i: number, auto = false): boolean => {
  ...
  for (let j = 1; j <= max; j++) {
    ...
    if (player.talismanLevels[i] < calculateMaxTalismanLevel(i)) {
      if (
        player.talismanShards >= priceMult * Math.max(0, Math.floor(1 + 1 / 8 * player.talismanLevels[i] ** 3))
      ) {
        checkSum++
      }
      if (
        player.commonFragments
          >= priceMult * Math.max(0, Math.floor(1 + 1 / 32 * (player.talismanLevels[i] - 30) ** 3))
      ) {
        checkSum++
      }
      if (
        player.uncommonFragments
          >= priceMult * Math.max(0, Math.floor(1 + 1 / 384 * (player.talismanLevels[i] - 60) ** 3))
      ) {
        checkSum++
      }
      if (
        player.rareFragments
          >= priceMult * Math.max(0, Math.floor(1 + 1 / 500 * (player.talismanLevels[i] - 90) ** 3))
      ) {
        checkSum++
      }
      if (
        player.epicFragments
          >= priceMult * Math.max(0, Math.floor(1 + 1 / 375 * (player.talismanLevels[i] - 120) ** 3))
      ) {
        checkSum++
      }
      if (
        player.legendaryFragments
          >= priceMult * Math.max(0, Math.floor(1 + 1 / 192 * (player.talismanLevels[i] - 150) ** 3))
      ) {
        checkSum++
      }
      if (
        player.mythicalFragments
          >= priceMult * Math.max(0, Math.floor(1 + 1 / 1280 * (player.talismanLevels[i] - 150) ** 3))
      ) {
        checkSum++
      }
    }
    ...
    if (checkSum === 7) {
      ...
      player.commonFragments -= priceMult
        * Math.max(0, Math.floor(1 + 1 / 32 * (player.talismanLevels[i] - 30) ** 3))
      player.uncommonFragments -= priceMult
        * Math.max(0, Math.floor(1 + 1 / 384 * (player.talismanLevels[i] - 60) ** 3))
      player.rareFragments -= priceMult
        * Math.max(0, Math.floor(1 + 1 / 500 * (player.talismanLevels[i] - 90) ** 3))
      player.epicFragments -= priceMult
        * Math.max(0, Math.floor(1 + 1 / 375 * (player.talismanLevels[i] - 120) ** 3))
      player.legendaryFragments -= priceMult
        * Math.max(0, Math.floor(1 + 1 / 192 * (player.talismanLevels[i] - 150) ** 3))
      player.mythicalFragments -= priceMult
        * Math.max(0, Math.floor(1 + 1 / 1280 * (player.talismanLevels[i] - 150) ** 3))
