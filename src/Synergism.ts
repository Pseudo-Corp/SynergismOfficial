...
const loadSynergy = (saveString: string): boolean => {
  ...
  if (data) {
    ...
    for (let buyAmountType of buyAmountTypes) {
      for (let buyAmountName of buyAmountNames) {
        DOMCacheGetOrSet(`${buyAmountType}${buyAmountName}`).classList.remove('toggled')
      }
      const curBuyAmount = player[`${buyAmountType}buyamount` as const]
      const curName = buyAmountNames[curBuyAmount.toString().length - 1]
      DOMCacheGetOrSet(`${buyAmountType}${curName}`).classList.add('toggled')
    }
