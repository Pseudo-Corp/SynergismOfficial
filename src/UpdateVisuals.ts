...
export const visualUpdateBuildings = () => {
  ...
  if (G.buildingSubTab === "coin") {
    ...
    if (player.resettoggle2 === 2) {
      ...
      DOMCacheGetOrSet(
        "autotranscend"
      ).textContent = `Transcend when the autotimer is at least ${
    ...
    for (let i = 1; i <= 5; i++) {
      const ascendBuildingI = `ascendBuilding${
        i as 1 | 2 | 3 | 4 | 5
      }` as const;
  ...
  if (G.runescreen === "runes") {
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      if (runeLevel < maxLevel) {
        ...
        DOMCacheGetOrSet(`rune${i}exp`).textContent =
          i18next.t("runes.maxLevel");
      }
      if (i <= 5) {
        ...
        DOMCacheGetOrSet(`bonusrune${i}`).textContent =
          i18next.t("runes.bonusNope");
      }
  ...
  if (G.runescreen === "talismans") {
    for (let i = 0; i < 7; i++) {
      ...
      DOMCacheGetOrSet(`talisman${i + 1}level`).textContent = `${
        player.ascensionCount > 0 ? "" : "Level "
      } ${format(player.talismanLevels[i])}/${format(maxTalismanLevel)}`;
    }
  }
...
export const visualUpdateCubes = () => {
  ...
  for (const name of names) {
    ...
    DOMCacheGetOrSet(`${name}QuarksOpenRequirement`).style.color =
      Math.max(1, toNextQuark[name]) === 1 ? "gold" : "white";
  }
  ...
  switch (player.subtabNumber) {
    ...
    case 4:
      DOMCacheGetOrSet("cubeAmount2").textContent = `You have ${format(
        player.wowCubes,
        0,
        true
      )} Wow! Cubes =)`;
    ...
    case 6:
      ...
      DOMCacheGetOrSet("heptGridOrbEffect").textContent = `${format(
...
export const visualUpdateCorruptions = () => {
  ...
  DOMCacheGetOrSet("autoAscend").innerHTML =
  ...
  DOMCacheGetOrSet("corruptionAscensionCount").style.display =
    ascCount > 1 ? "block" : "none";
...
export const visualUpdateOcteracts = () => {
  ...
  DOMCacheGetOrSet("secondsPerOcteract").style.display =
    perSecond < 1 ? "block" : "none";
  ...
  DOMCacheGetOrSet("octeractPerSeconds").style.display =
    perSecond >= 1 ? "block" : "none";
  ...
  DOMCacheGetOrSet("totalOcteractCubeBonus").style.display =
    cTOCB >= 0.001 ? "block" : "none";
  DOMCacheGetOrSet("totalOcteractQuarkBonus").style.display =
    cTOQB >= 0.001 ? "block" : "none";
  DOMCacheGetOrSet("totalOcteractOfferingBonus").style.display =
    cTOOB >= 0.001 ? "block" : "none";
  DOMCacheGetOrSet("totalOcteractObtainiumBonus").style.display =
    cTOOOB >= 0.001 ? "block" : "none";
...
export const visualUpdateAmbrosia = () => {
  ...
  const luckBonusPercent =
    100 * (player.caches.ambrosiaLuckAdditiveMult.totalVal - 1);
  ...
  const availableBlueberries =
    player.caches.blueberryInventory.totalVal - player.spentBlueberries;

  const extraLuckHTML =
    luckBonusPercent > 0.01
      ? `[<span style='color: var(--amber-text-color)'>☘${format(
          baseLuck,
          0,
          true
...
export const visualUpdateShop = () => {
  ...
  for (const key of keys) {
    ...
    if (shopItem.type === shopUpgradeTypes.CONSUMABLE) {
      ...
      switch (player.shopBuyMaxToggle) {
        case false:
          ...
          break;
        case "TEN":
          el.textContent = `+${Math.min(10, maxBuyablePotions)} for ${format(
          ...
          break;
        default:
          el.textContent = `+${maxBuyablePotions} for ${format(
    ...
    if (shopItem.type === shopUpgradeTypes.UPGRADE) {
      ...
      switch (player.shopBuyMaxToggle) {
        case false:
          DOMCacheGetOrSet(`${key}Button`).textContent =
            player.shopUpgrades[key] >= shopItem.maxLevel
  ...
  DOMCacheGetOrSet("buySingularityQuarksAmount").textContent = `${
    player.goldenQuarks < 1000 ? "Owned: " : ""
  }${format(player.goldenQuarks)}`;
  DOMCacheGetOrSet("buySingularityQuarksButton").textContent = `Buy! ${format(
