  ...
  switch (input) {
    ...
    case "calculator5":
      lol.innerHTML = i18next.t("shop.upgradeEffects.calculator5", {
        ...
        amount2:
          Math.floor(player.shopUpgrades.calculator5 / 10) +
    ...
    case "calculator6":
      lol.innerHTML = i18next.t("shop.upgradeEffects.calculator6", {
        ...
        amount2:
          player.shopUpgrades.calculator6 === shopData.calculator6.maxLevel
            ? 24
            : 0,
      });
      break;
    case "calculator7":
      lol.innerHTML = i18next.t("shop.upgradeEffects.calculator7", {
        ...
        amount2:
          player.shopUpgrades.calculator7 === shopData.calculator7.maxLevel
            ? 48
            : 0,
      });
      break;
...
export const friendlyShopName = (input: ShopUpgradeNames) => {
  ...
  const names: Record<ShopUpgradeNames, string> = {
    ...
    shopAmbrosiaLuckMultiplier4:
      "The Fourth Multiplicative Ambrosia Luck Multiplier",
...
export const buyShopUpgrades = async (input: ShopUpgradeNames) => {
  ...
  if (player.shopUpgrades[input] >= shopItem.maxLevel) {
    return player.shopConfirmationToggle
      ? Alert(
          `You can't purchase ${friendlyShopName(
            input
          )} because you are already at the maximum ${
  ...
  if (player.shopBuyMaxToggle === "ANY" && !singular) {
    const buyInput = await Prompt(
      `You can afford to purchase up to ${merch} of ${friendlyShopName(
        input
      )} for ${buyCost.toLocaleString()} Quarks. How many would you like to buy?${
        maxPots + noRefunds
      }`
    );
    let buyAny: number;
    if (
      ...
      const other =
        input === "offeringPotion" ? "obtainiumPotion" : "offeringPotion";
  ...
  if (
    ...
    p = await Confirm(
      `You are about to ${
        ...
        input
      )} for ${buyCost.toLocaleString()} Quarks. Press 'OK' to finalize purchase.${
        maxPots + noRefunds
      }`
    );
  }
...
export const useConsumable = async (
  ...
  const p =
    player.shopConfirmationToggle && !automatic
  ...
  if (p || ignoreBoolean) {
    ...
    for (const shopItem in shopData) {
      ...
      if (
        ...
        const doNotRefund =
          item.price * item.refundMinimumLevel +
          (item.priceIncrease *
...
export const getQuarkInvestment = (upgrade: ShopUpgradeNames) => {
  ...
  const val =
    shopData[upgrade].price * player.shopUpgrades[upgrade] +
